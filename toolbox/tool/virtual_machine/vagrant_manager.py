...
class VagrantManager(AbstractManager):
    ...
    def start(self):
        if self.status(output=False):
            logger.info('VM %s is up, nothing to do here', self.virtual_machine.name)
            return
    ...
    def status(self, output: bool = True):
        ...
        if output:
            if len(list(status)) == 1:
                logger.info('VM %s is up', self.virtual_machine.name)
            else:
                logger.info('VM %s is down', self.virtual_machine.name)
